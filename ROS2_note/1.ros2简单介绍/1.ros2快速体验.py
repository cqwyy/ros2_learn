...
# colcon build --packages-select 包名 #编译，不选择包就全编译了，很慢
...
# cpp
# 终端下，进入ws00_helloworld/src目录，使用如下指令创建一个C++功能包：（创建完后退回工作空间）
# ros2 pkg create（创建功能包） pkg01_helloworld_cpp（包名） --build-type（指定包的具体类型（构建类型）） ament_cmake（cpp要用的的类型） --dependencies(添加依赖) rclcpp(ros2的cpp的客户端,包含了一些API) --node-name（设置一个可执行程序，在功能包下面会自动生成一个同名的原代码文件，这里不要加后缀，会自动生成） helloworld
# 执行完毕，在src目录下将生成一个名为pkg01_helloworld_cpp的目录（包），且目录中已经默认生成了一些子级文件与文件夹。
# TODO 添加依赖是添加的依赖的功能包，可以是系统给的，也可以是自定义的，只要是个功能包且能被检索到，
# TODO 创建功能包时候顺便创建一个节点，这样cmake里面有配置信息，不用自己写，后面要写也可以复制粘贴。创建完要把终端返回到工作空间目录下
# 主要是会自动生成cmakelists以及.xml文件，xml注意第十二左右
# < !-- 所需要依赖 -->  即功能包依赖哪些外部库  create那指定的,需要其他依赖就再按照这个格式添加多行
# < depend > rclcpp < / depend >
# cmakelists中
# 引入外部依赖包--需要其他的也可以手动添加 （depend和find_package以及ament_target_dependencies对应）
# find_package(rclcpp REQUIRED)
# 映射源文件与可执行文件
# add_executable(helloworld src/helloworld.cpp)   直译为添加可执行的，参数二对应源文件，参数一为这个文件生成一个可执行程序，程序名为参数一（一般默认设置为和文件名一样，就是create时候最后那个参数）
# 设置目标依赖库（为可执行程序添加依赖）
# ament_target_dependencies(--其他需要的手动添加
#   helloworld  ---生成的可执行程序
...
# TODO 功能包创建：在工作空间的src目录下，编译什么的都在工作空间下，所以创建完功能包要及时返回工作空间
...
#TODO 添加依赖xml的depend和cmake的find_package以及ament_target_dependencies对应
# TODO 添加节点，cmakelists里add_excetable新建可执行文件再多连接一个cpp文件  ament_target_dependencies里让新的可执行文件连接依赖，最后生成目录的生成名也改成新的（这里还是不用新的目录了，可以在install里添加多个可执行程序，都放到一个目录里）
...
# TODO 文件名与可执行程序名设置为一样的，这样知道可执行程序名是那个文件生成的（可执行程序名是cmakelist配置的，但是最好是和文件名一样吧）（一般一个程序就一个节点）
# TODO 创建功能包时候顺便创建一个节点，这样cmake里面有配置信息，不用自己写，后面要写也可以复制粘贴。创建完把终端返回到工作空间目录下

# TODO 新建文件夹都要有一步操作：例如launch，cmakalists中加一个install(DIRECTORY launch DESTINATION share/${PROJECT_NAME})
